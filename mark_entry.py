...
class Mark_Entry(Toplevel):
    ...
    def selected_exam(self,event):
        ...
        self.std_label = Label(self.lf2, text="Standard", bg=self.bgclr1, fg="black",  bd=5, font=(self.f1, 15),
                              relief=GROOVE)
        self.std_label.place(x=500, y=10)
        ...
        self.standard_entry.place(x=650, y=10, height=30)
        ...
        for i in range(len(self.subject)-1):
            ...
            if i % 2 == 0:
                ...
                top += 50
        self.confirm_btn= Button(self.lf2,text="Confirm",bg=self.bgclr2, bd=5, font=(self.f2, 15),command=self.set_mark)
        self.confirm_btn.place(x=450, y=450)
        self.reset_btn = Button(self.lf2, text="Reset",bg=self.bgclr2, bd=5, font=(self.f2, 15), command=self.reset)
...
##===================================================frame 2============================================================
        self.lf2 = LabelFrame(self, text="MARK'S ENTRY", bd=2, fg="black", bg=self.bgclr1, font=(self.f1, 20), relief=GROOVE)
        ...
        self.cb1.place(x=320, y=10, height=30)
        query = "select data from exams"
        j_data = self.conn.execute(query).fetchone()
        data = json.loads(j_data[0])
        exams = data.keys()
